...
Regenerate it after changing the Mermaid block. `mermaid-cli` needs a Chromium, which the `node`
container doesn't have — but Puppeteer's cached one on the host does the job:

```bash
PUPPETEER_EXECUTABLE_PATH=$(find ~/.cache/puppeteer/chrome-headless-shell -type f -name chrome-headless-shell | sort | tail -1) \
  npx -y @mermaid-js/mermaid-cli -i docs/database.mmd -o docs/database.svg
```
