...
- **Below `sm` a dialog is a bottom sheet; from `sm` up it's a centred card.** The sheet is anchored to the bottom edge, `rounded-t-2xl`, and its height follows its content up to `88dvh` — so a two-line confirm is two lines tall, not a full screen with its buttons stranded mid-display.
- **`DialogFooter` sits at the end of the content, not stuck to the scrollport.** It was `sticky bottom-0`, which meant that when the keyboard opened and the sheet shrank, the buttons rode up and hovered above the keys — the sheet resizing under them made them read as chasing the keyboard rather than belonging to the form. Content-height sheets put them on screen without scrolling for most dialogs anyway.
- **Don't let Radix auto-focus an edit form.** It focuses the first tabbable element on open, which opens the keyboard and covers half the sheet. That's right when creating (you're about to type) and wrong when editing, where the row was opened to change one specific field. Pass `onOpenAutoFocus={(e) => e.preventDefault()}` for the edit case — focus falls back to the dialog container, so Escape, tab order and screen-reader announcement all still work.
