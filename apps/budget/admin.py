...
class GoalInline(StackedInline):
    model = Goal
    extra = 0
...
class CategoryAdmin(ModelAdmin):
    list_display = ["__str__", "category_type", "budget", "monthly_budget", "is_goal_display"]
    list_filter = ["category_type", "budget"]
    search_fields = ["name", "parent__name"]
    autocomplete_fields = ["parent"]
    list_select_related = ["parent", "budget", "goal"]
    inlines = [SubcategoryInline, GoalInline]
